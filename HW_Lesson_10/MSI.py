...
class MSI(Lenovo):
    def __init__(self, model: str, screen_diagonal: int or float, screen_rate: int, cpu: str, gpu: int, ssd: int,
                 ram: int, operating_system: str, discount: int, additional_guarantee: str):
        super().__init__(model, screen_diagonal, screen_rate, cpu, gpu, ssd, ram, operating_system, discount)
        self.__price = 0
        self._additional_guarantee = additional_guarantee
        self._model_options = {"Titan": 500, "Raider": 400}
        ...
        self._additional_guarantee_option = {"1 year": 35, "2 year": 60}
    ...
    def __additional_guarantee(self):
        if self._additional_guarantee not in self._additional_guarantee_option.keys():
            raise TypeError("Error! Check your data and try again!")
        else:
            self.__price += self._additional_guarantee_option.get(self._additional_guarantee)
    ...
    def help():
        print("""Create your own notebook by entering the parameters from the following:
                  ...
                  Additional_guarantee: 1 year, 2 year\n""")


if __name__ == '__main__':
    first = MSI("Raider", 16, 120, "Intel i9", 32, 2048, 64, "Windows", 0, "2 year")
    first.help()
    print(first.price)

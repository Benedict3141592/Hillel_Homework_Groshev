...
class Acer(HP):
    def __init__(self, model: str, screen_diagonal: int or float, screen_rate: int, cpu: str, gpu: int, ssd: int,
                 ram: int, logo: bool, operating_system: str, anti_flooding_system: bool, mouse: str):
        super().__init__(model, screen_diagonal, screen_rate, cpu, gpu, ssd, ram, logo, operating_system,
                         anti_flooding_system)
        self._mouse = mouse
        self.__price = 0
        self._model_options = {"ConceptD": 350, "Predator": 250}
        self._screen_diagonal_options = {13.4: 50, 15.6: 75, 16: 100}
        ...
        self._mouse_option = {"Acer": 35, "Razer": 45, "Logitech": 40}
    ...
    def __add_mouse(self):
        if self._mouse not in self._mouse_option.keys():
            raise TypeError("Error! Check your data and try again!")
        else:
            self.__price += self._mouse_option.get(self._mouse)
    ...
    def help():
        print("""Create your own notebook by entering the parameters from the following:
              Model: ConceptD, Predator
              Screen diagonal: 13.4, 15.6, 16
              ...
              Mouse: Acer, Razer, Logitech\n""")


if __name__ == '__main__':
    first = Acer("Predator", 15.6, 120, "Intel i9", 32, 2048, 64, False, "Windows", True, "Razer")
    first.help()
    print(first.price)
